...
(define levels-timer (list 20 90 75 85 80 75 75 60))
...
(define (level-1-frame-update x y inp)
   (cond ;[(> diff 5) "LEVEL_1-SOLN"]
         [(and (mouse=? "button-down" inp)
              (((button-area 97.5 24.5) 850 670) x y) ) "WELCOME"]
...
(define (congrats-frame-update x y inp)
  (cond [(((button-area 97.5 24.5) 503 496) x y) (frame-update "next" inp "CONGRATULATIONS" "LEVEL-1" (lambda (x) (set! next_image x)) next_hover next_press (lambda () (begin
        ...
        [(((button-area 97.5 24.5) 781 627) x y) (frame-update "quit" inp "CONGRATULATIONS" "WELCOME" (lambda (x) (set! quit_image x)) quit_hover quit_press (lambda () (begin
                                                                                                                                                         (send timer-1 stop)
                                                                                                                                                         (reset-timer)
                                                                                                                                                         (set! state-new (+ 1 state-new))
                                                                                                                                                         )))]
...
(define (gameover-frame-update x y inp)
  ...
  (cond [(((button-area 97.5 24.5) 480 554) x y) (frame-update "retry" inp "GAMEOVER" "LEVEL-1" (lambda (x) (set! retry_image x)) retry_hover retry_press (lambda () (begin (updates)
        ...
        [(((button-area 97.5 24.5) 480 644) x y) (frame-update "quit" inp "GAMEOVER" "WELCOME" (lambda (x) (set! quit_image x)) quit_hover quit_press (lambda () (begin
                                                                                                                                                   (send timer-1 stop)
                                                                                                                                                         (reset-timer)
                                                                                                                                                         )))]
        [(and (not (equal? down-status "NO"))
        ...
        [(mouse=? inp "move") (begin (reset-all)
                                     "GAMEOVER")]
        [else "GAMEOVER"]))
(define (welcome-frame-update x y inp)
  (cond [(((button-area 157 40) 220 600) x y) (frame-update "play" inp "WELCOME" "LEVEL-1" (lambda (x) (set! play-levels_image x)) play-levels_hover play-levels_press (lambda () (begin (send timer-1 stop)
                                                                                                                                                                        (reset-timer)
                                                                                                                                                                        (updates)
                                                                                                                                                                        (work 10)
                                                                                                                                                                        )))]
        [(((button-area 157 40) 720 600) x y) (frame-update "create" inp "WELCOME" "CREATE-LEVEL" (lambda (x) (set! create-level_image x)) create-level_hover create-level_press (lambda () (displayln 'happy)))]
        [(((button-area 67 32) 880 50) x y) (frame-update "back" inp "WELCOME" "GO-TO-HOME" (lambda (x) (set! back_image x)) back_hover back_press (lambda () (displayln "done")))]
        [(and (not (equal? down-status "NO"))
              (mouse=? inp "button-up")) (begin (reset-all)
                                                (set! down-status "NO") "WELCOME")]
        [(mouse=? inp "move") (begin (reset-all)
                                     "WELCOME")]
        [else "WELCOME"]))
...
(define (grid-update x y inp package stat)
  ...
  (cond [(and (equal? down-status "NO") (equal? stat "LEVEL-1")) (begin 
                                                                        (if (member (map (lambda (x) (press->normal (car x))) (remove-duplicates level-1-formed-words-list (lambda (x y) (equal? (cdr y) (cdr x)))))
                                                                            ...
                                                                            (begin (displayln level-1-word-list) (set! level-1-word-list (remove (list->string (map (lambda (x) (picture->char x)) (map (lambda (x) (press->normal (car x))) (remove-duplicates level-1-formed-words-list (lambda (x y) (equal? (cdr y) (cdr x))))))) level-1-word-list))
                                                                            ...
                                                                            (if (member (reverse (map (lambda (x) (press->normal (car x))) (remove-duplicates level-1-formed-words-list (lambda (x y) (equal? (cdr y) (cdr x))))))
                                                                                    level-1-image-word-list)
                                                                            (begin (displayln level-1-word-list)  (set! level-1-word-list (remove (list->string (reverse (map (lambda (x) (picture->char x)) (map (lambda (x) (press->normal (car x))) (remove-duplicates level-1-formed-words-list (lambda (x y) (equal? (cdr y) (cdr x)))))))) level-1-word-list))
                                                                                   (set! level-1-formed-words-list null)
                                                                                   (set! filled-level-1-images-list filled-level-1-pressed-images-list)
                                                                                   (set! new-image-list-1 (list-update filled-level-1-pressed-images-list
                                                                                                                       (index-of-image x y (list-ref package 2) 0)
...
(define (main-frame stat)
  (begin  ;(displayln stat)
          (cond [(equal? stat "LEVEL-1") (level-1_frame)]
               ...
               [(equal? stat "WELCOME") (welcome-frame)]
  ...
  (big-bang "WELCOME"
                       ...
                       [on-mouse (λ (stat x y inp)
                        (cond [(or (mouse=? inp "enter")
                              ...
                              [(equal? stat "WELCOME") (welcome-frame-update x y inp)]
                              [(equal? stat "CREATE-LEVEL") (take-level-details) (welcome-frame-update x y inp)]
                       ...
                       [on-tick (λ (stat) (cond [(> diff timer-timings) (begin (send timer-1 stop) "GAMEOVER")]
                                                [(and (null? level-1-word-list) (equal? stat "LEVEL-1") (equal? creation #t)) (send timer-1 stop) (set! creation #f) "WELCOME"]
...
;(define (game) (big-bang "level-1-soln"
;                 [name "WORD SEARCH"]
;                 [to-draw level-1_frame-soln]))
;(time (game-world))
;(game)
...
(define timer-1 (new timer%
                   [notify-callback (lambda ()
                                       ...
                                       ;(cond [(= diff 10.0) (begin (send timer-1 stop)
                                                                 ; (reset-timer)
                                                                  ;(work 10)
                                                                  )]
                                                                  ;(main-frame "LEVEL_1-SOLN"))]))]
                                      ;(final "s"))]
                                                                  ;(mainframe "gameover/retry"))]))]
                                       ;(mainframe "next-window"))]
                    [interval #f]))
